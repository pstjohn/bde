...
def covalent_radii(x):
    return PeriodicTable.GetRcovalent(pt, x)
...
class GaussianRunner(object):
    ...
    def process(self):
        ...
        with tempfile.TemporaryDirectory(dir=self.scratchdir) as tmpdirname:
            ...
            print(
                "starting SMILES {0} on host {1}".format(
                    self.smiles, socket.gethostname()
                )
            )
            ...
            print(
                "python walltime for SMILES {0} on host {1}: {2}".format(
    ...
    def optimize_molecule_mmff(self):
        ...
        NumConformers = np.clip(
            3 ** NumRotatableBonds, self.min_conformers, self.max_conformers
        )
        ...
        if len(conformers) == 1:
            logging.critical("Only 1 conformer for SMILES {}".format(self.smiles))
    ...
    def run_gaussian(self, tmpdirname):
        ...
        with tempfile.TemporaryDirectory(dir=tmpdirname) as gausstmp:
            ...
            subprocess.run(
                gaussian_cmd, shell=True, env=env, timeout=self.gaussian_timeout
            )
    ...
    def parse_log_file(self):
        ...
        assert np.allclose(
            np.array([a.GetAtomicNum() for a in mol.GetAtoms()]), data.atomnos
        ), "Stoichiometry check failed"
        ...
        for bond in mol.GetBonds():
            ...
            max_length = (
                covalent_radii(bond.GetBeginAtom().GetSymbol())
                + covalent_radii(bond.GetEndAtom().GetSymbol())
                + 0.4
            )
